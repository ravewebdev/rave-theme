...
function r_a_v_e_posted_on( $echo = true ) {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'r_a_v_e' ),
...
function r_a_v_e_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && r_a_v_e_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'r_a_v_e' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'r_a_v_e' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'r_a_v_e' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
